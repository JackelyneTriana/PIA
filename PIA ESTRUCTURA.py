...
def menu():
    ...
    print("6) Estadisticos descriptivos de materias")
...
def Opcion_6():
    materias = conn.cursor()
    nombre = conn.cursor()
    materias.execute("SELECT idmateria FROM calificaciones GROUP BY idmateria")
    nombre.execute("SELECT nombre FROM materias")
    materias_ = materias.fetchall()
    nombre_ = nombre.fetchall()
    promedios = []
    for idmat, nombre in zip(materias_, nombre_):
        clave = int(idmat[0])
        califs = conn.cursor()
        valores = {"idmateria":clave}
        califs.execute("SELECT calificacion FROM calificaciones WHERE idmateria=:idmateria", valores)
        calif = califs.fetchall()
        print(f"{nombre[0]}")
        print("Promedio: \t\t", end="")
        promedios.append(np.mean(calif))
        print(np.mean(calif))
        print("Mediana: \t\t", end="")
        print(np.median(calif))
        print("Desviacion estandar: \t", end="")
        print(np.std(calif))
        print("Calificacion minima: \t", end="")
        print(np.min(calif))
        print("Calificacion maxima: \t", end="")
        print(np.max(calif))
        print("\n")
    promedio_minimo = min(promedios)
    indice = promedios.index(promedio_minimo)
    nombremateria = nombre_[indice]
    print(f"La materia con mas bajo rendimiento es {nombremateria[0]} con un promedio de {promedio_minimo}")
    print(separador)
    menu()
